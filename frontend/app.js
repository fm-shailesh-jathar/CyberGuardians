...
function auditMakeResults(questions) {
  ...
  for (const q of questions) {
    ...
    if (!best || best.score < SA_AUDIT_AUTO_THRESHOLD) {
      out.push({
        question: q,
        suggestedAnswer: 'No answer found in knowledge base. Manual response required.',
        confidence: best ? best.score : 0,
        sourceQuestion: best ? best.entry.question : '',
        status: 'Manual'
      });
    } else {
      out.push({
...
function renderAuditResults() {
  ...
  const rows = audit.results.filter(r => saAuditResultFilter === 'all' ? true : r.status.toLowerCase() === saAuditResultFilter);
  tbl.innerHTML = rows.map(r => {
    const badge = r.status === 'Auto' ? 'b-green' : 'b-amber';
    return `<tr>
      <td style="color:var(--text2)">${saEscapeHtml(r.question)}</td>
      <td>${saEscapeHtml(r.suggestedAnswer)}</td>
